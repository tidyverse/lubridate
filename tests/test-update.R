...
test_that("update returns NA for date-times in the spring dst gap",{ 
	...
	expect_that(update(poslt, seconds = 61), equals(NA))
	expect_that(update(poslt, minutes = 61), equals(NA))
	expect_that(update(poslt, hours = 2), equals(NA))
	
	poslt <- as.POSIXlt("2010-03-13 02:59:59", tz = "UTC", format
		 = "%Y-%m-%d %H:%M:%S")
	poslt <- force_tz(poslt, "")
	
	expect_that(update(poslt, mday = 14), equals(NA))
	expect_that(update(poslt, wday = 8), equals(NA))
	expect_that(update(poslt, yday = 73), equals(NA))
	
	poslt <- as.POSIXlt("2010-02-14 02:59:59", tz = "UTC", format
		 = "%Y-%m-%d %H:%M:%S")
	poslt <- force_tz(poslt, "")
	
	expect_that(update(poslt, months = 3), equals(NA))
	
	poslt <- as.POSIXlt("2009-03-14 02:59:59", tz = "UTC", format
		 = "%Y-%m-%d %H:%M:%S")
	poslt <- force_tz(poslt, "")
	
	expect_that(update(poslt, years = 2010), equals(NA))
	
	poslt <- as.POSIXlt("2010-03-14 02:59:59", tz = "UTC", format
		 = "%Y-%m-%d %H:%M:%S")
	
	expect_that(update(poslt, tz = ""), equals(NA))
})
