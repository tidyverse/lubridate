...
test_that("update.Date performs simple operation as expected", {
  ...
  expect_match(tz(update(date, tz = "UTC")), "UTC")
})
...
test_that("update.POSIXt performs simple operation as expected", {
  ...
  expect_match(tz(update(poslt, tz = "UTC")), "UTC")
  ...
  expect_match(tz(update(posct, tz = "UTC")), "UTC")
})
...
test_that("update.POSIXt works on wdays", {
  ...
  ws <- 1
  expect_equal(wday(update(ct, wday = 1, week_start = ws), week_start = ws), 1)
  expect_equal(wday(update(ct, wday = 2, week_start = ws), week_start = ws), 2)
  expect_equal(wday(update(ct, wday = 5, week_start = ws), week_start = ws), 5)
  expect_equal(wday(update(ct, wday = 7, week_start = ws), week_start = ws), 7)
  expect_equal(wday(update(date, wday = 1, week_start = ws), week_start = ws), 1)
  expect_equal(wday(update(date, wday = 2, week_start = ws), week_start = ws), 2)
  expect_equal(wday(update(date, wday = 5, week_start = ws), week_start = ws), 5)
  expect_equal(wday(update(date, wday = 7, week_start = ws), week_start = ws), 7)
...
test_that("update performs roll overs correctly for Date objects", {
  ...
  expect_match(tz(update(date, month = 13)), "UTC")
})
...
test_that("update performs roll overs correctly for POSIXlt objects", {
  ...
  expect_match(tz(update(poslt, month = 13)), "GMT")
})
...
test_that("update performs roll overs correctly for POSIXct objects", {
  ...
  expect_match(tz(update(posct, month = 13)), "GMT")
})
...
test_that("update returns NA for date-times in the spring dst gap", {
  ...
  expect_that(is.na(update(poslt, minutes = 65)), is_true())
