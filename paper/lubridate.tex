...
While base \proglang{R} chandles some of these problems, the syntax it uses is uninviting, often confusing, and difficult to remember. Moreover, the correct \proglang{R} code often changes depending on the class of date-time object being used. \pkg{lubridate} acknowledges these problems and makes it easier to work with date-time data in \proglang{R}. Specifically, \pkg{lubridate} helps users:
...
\pkg{lubridate} uses an intuitive user interface inspired by the date libraries of object oriented programming languages.  \pkg{lubridate} methods are compatible with a wide-range of common date-time objects. These include character strings, POSIXct, POSIXlt, Date, chron ~\cite{chron}, fCalendar ~\cite{fCalendar}, zoo ~\cite{zoo}, xts ~\cite{xts}, its ~\cite{its}, tis ~\cite{tis}, timeSeries ~\cite{timeSeries}, fts ~\cite{fts}, and tseries ~\cite{tseries} objects. Note that \pkg{lubridate} overrides the + and - methods for POSIXt, Date, and difftime objects in base \proglang{R}.
...
\pkg{lubridate} can be downloaded at \url{http://cran.r-project.org/}, and its development site is \url{http://github.com/hadley/lubridate}.
...
Figure~\ref{fig:comparison} provides a more complete comparison between \pkg{lubridate} methods and base \proglang{R} methods. It shows how \pkg{lubridate} can simplify each of the common date time tasks presented in RNews Volume 4/1, June 2004. It also provides a useful summary of \pkg{lubridate} methods.
...
\pkg{lubridate} allows math with both relative and exact units by introducing four new time related objects. These are \emph{instants}, \emph{intervals}, \emph{durations}, and \emph{periods}, terminology borrowed from the \proglang{JODA} time project. 
...
First we'll examine when during the year the Lakers have games. We choose to use the \pkg{ggplot2} ~\cite{ggplot2} package to create our graphs.\\
