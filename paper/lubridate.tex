...
\begin{itemize}
  ...
  \item Perform accurate math on date-times and timespans, Sections~\ref{sec:types} and~\ref{sec:rounding}.
...
Note that \pkg{lubridate} overrides the + and - methods for POSIXt, Date, and difftime objects in base \proglang{R}. This allows users to perform simple arithmetic on date-time objects with the new timespan classes introduced by \pkg{lubridate}, but it does not alter the way \proglang{R} implements addition and subtraction for non-\pkg{lubridate} objects.

\pkg{lubridate} introduces four new object classes based on the \proglang{Java} language \pkg{Joda Time} project \citep{jodatime}. joda time introduces a conceptual model of the different ways to measure timespans. Section~\ref{sec:types} describes this model and explains how \pkg{lubridate} uses it to perform easy and accurate arithmetic with dates. 
...
\code{[1] 2010-01-01 12:00:00 -- 2011-01-01 12:00:00}\\

We can access the start and end dates of an interval with \code{start()} and \code{end()}. \pkg{lubridate} provides each of these with an S3 method for interval objects. Intervals always begin at the date-time that occurs first and end at the date-time that occurs last. Hence, intervals always have a positive length.\\

\code{R> start(span)}\\
\code{[1] "2010-01-01 12:00:00 UTC"}\\
\code{R> end(span)}\\
...
Adding intervals to other date-times won't produce an error message. Instead \pkg{lubridate} will coerce the interval to a duration object, which is like an interval but without the reference dates. See Section~\ref{sec:durations}.

\code{R> start_2011 + span}\\
\code{coercing interval to duration}\\
...
\code{R> as.interval(difftime(start_2011, start_2010), ymd("2010-03-05"))}\\
\code{[1] 2010-03-05 -- 2011-03-05 }\\
...
The length of a duration is invariant to leap years, leap seconds, and daylight savings time because durations are measured in seconds. Hence, durations have consistent lengths and can be easily compared to other durations. Durations are the appropriate object to use when comparing time based attributes, such as speeds, rates, and lifetimes. \code{difftime} objects from base \proglang{R} are one type of duration object. \pkg{lubridate} provides a second type: duration class objects. These objects can be used with other date-time objects without worrying about what units they are displayed in. A duration object can be created with the function \code{new_duration()}:\\

\code{R> new_duration(60)}\\
\code{[1] 60s}\\
...
\code{dyears()}, \code{dweeks()}, \code{ddays()}, \code{dhours()}, \code{dminutes()}, and  \code{dseconds()}. The d in the title stands for duration and distinguishes these objects from period objects, which are discussed in Section~\ref{sec:periods}. Each object creates a duration in seconds using the estimated relationships given above. The argument of each function is the number of estimated units we wish to include in the duration. For example,\\
...
\code{R> c(1:3) * dhours(1) }\\
\code{[1]  3600s (1h)   7200s (2h)  10800s (3h) }\\

Durations can be added and subtracted to any instant object. For example,\\

\code{R> start_2011 + dyears(1)}\\
\code{[1] "2012-01-01 12:00:00 UTC"}\\

\code{R> start_2012 <- ymd_hms("2012-01-01 12:00:00")}\\
\code{R> start_2012 + dyears(1)}\\
...
\code{R> dweeks(1) + ddays(6) + dhours(2) + dminutes(1.5) + dseconds(3)}\\
\code{[1] 1130493s (13.08d) }\\

We can also create durations from intervals and periods using \code{as.duration()}. 

\code{R> as.duration(span)}\\
\code{[1] 31536000s (365d) }\\
...
We can also convert other timespans to period objects with the function \code{as.period()}.\\
...
\code{R> halloween <- ymd("2010-10-31")}\\
\code{R> christmas <- ymd("2010-12-25")}\\
\code{R> interval <- new_interval(halloween, christmas)}\\
\code{[1] 2010-10-31 -- 2010-12-25}\\

Since durations are an exact measurement of a timespan, we can divide this interval by a duration to get an exact answer.\\

\code{R> interval / dweeks(1)}\\
\code{[1] 7.857143}\\

Intervals are also exact measures of timespans. Although it is more work, we could divide an interval by another interval to get an exact answer. This gives the same answer as above because \pkg{lubridate} automatically coerces intervals in the denominator to durations.\\

\code{R> interval / new_interval(halloween, halloween + weeks(1))}\\
\code{interval denominator coerced to duration}\\
\code{[1] 7.857143}\\

Dividing by periods returns an integer value equal to the number of whole periods that occur in the timespan. This is equivalent to the $\%/\%$ operator. \\

\code{R> interval / weeks(1)}\\
\code{[1] 7}\\
\code{R> interval \%/\% weeks(1)}\\
\code{[1] 7}\\

Since periods have inconsistent lengths, we can not express the remainder as a decimal. For example, if we were to divide our interval by months, the remainder would be 24 days.\\

\code{R> interval / months(1)}\\
\code{[1] 1}\\
\code{R> interval \%\% months(1)}\\
\code{[1] 24 days}\\

Should we calculate 24 days as $24 / 30 = 0.8$ months since November has 30 days? Or should we calculate 24 days as $24 / 31 = 0.77$ months since December has 31 days? Both November and December are included in our numerator. If we want an exact calculation, we should use durations instead of periods. As shown above, we can retrieve the remainder using the $\%\%$ operator.\\
...
\code{force_tz()} does the opposite of \code{with_tz()}: it changes the instant of time that is displayed, but the clock time remains the same. For example, the code below moves us to a new instant that occurs 6 hours earlier.\\
...
\code{with_tz()} and \code{force_tz()} ony work with time zones recognized by your computer's operating system. This list of time zones will vary between computers. See the base \proglang{R} help page for \code{Sys.timezone()} for more information.
...
\code{R> lakers$time <- dminutes(c(12, 24, 36, 48, 53)[lakers$period]) - lakers$time}\\
...
\code{R> qplot(time, data = lakers, geom = "histogram", binwidth = 60)}\\
...
\begin{figure}[htpb]
  ...
  \caption{The graph on the right displays seconds on the x axis. The graph on the left uses a more intuitive division with minutes.}
  \label{fig:plays}
\end{figure}

We see that the number of plays peaks within each of the four periods and then plummets at the beginning of the next period, figure~\ref{fig:plays}. The most plays occur in the last minute of the game. Perhaps any shot is worth taking at this point or there's less of an incentive not to foul other players. Less plays occur in overtime, since not all games go to overtime.
...
\code{R> game1 <- lakers[lakers$date == ymd("20081225"),]}\\
...
\begin{figure}[htpb]
  ...
  \caption{Distribution of wait times between shot attempts. Rarely did 30 seconds pass without a shot attempt.}
  \label{fig:waits}
\end{figure}
...
Future work on the \pkg{lubridate} package will develop methods for handling partial dates and for modeling recurrent events, such as stock market opening times, business days, or street cleaning hours. In particular, we hope to create methods for R that work with reoccurring temporal date patterns, which were introduced by Martin Fowler \citep{fowler} and have been implemented in \proglang{Ruby} by the \proglang{runt} project \citep{runts}.
